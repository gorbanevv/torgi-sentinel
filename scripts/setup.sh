...
if [ -n "$TG_TOKEN" ] && [ -n "$TG_CHAT" ]; then
  ...
  node -e "const fs=require('fs');const c=JSON.parse(fs.readFileSync('config.example.json','utf8'));c.telegramBotToken=process.env.TG_TOKEN;c.telegramChatId=process.env.TG_CHAT;if(process.env.TG_API){c.telegramApiBase=process.env.TG_API;c.heartbeatUrl=process.env.TG_API.replace(/\/$/,'')+'/heartbeat';}fs.writeFileSync('config.json',JSON.stringify(c,null,2));console.log('   ok, фильтров:',c.filters.length, process.env.TG_API?('| релей+heartbeat '+process.env.TG_API):'| напрямую в Telegram');"
